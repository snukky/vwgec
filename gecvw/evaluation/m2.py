...
class M2Evaluator(object):
    def __init__(self, m2_file, cword_file, pred_file, txt_file=None, work_dir=None):
        self.i = 0
        self.m2_file = m2_file
        self.cword_file = cword_file
        self.pred_file = pred_file
        self.cset = CSet(config['target-cset'])

        self.work_dir = work_dir or '.'
        if not os.path.exists(self.work_dir):
            raise Exception("Directory does not exist: {}".format(self.work_dir))
    ...
    def run(self, thr):
        ...
        self.apply_predictions(self.txt_file, out, self.cword_file, self.pred_file)
    ...
    def apply_predictions(self, txt_file, out_file, cword_file, pred_file):
        pred_iter = PredictionIterator(
            txt_file, cword_file, pred_file, cset=self.cset)
