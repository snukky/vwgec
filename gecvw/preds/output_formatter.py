...
class OutputFormatter(object):
    def __init__(self, output, threshold=0.0, restore_case=False):
        self.output = output
        self.threshold = 0.0
    ...
    def format(self, n, sentence, preds):
        ...
        new_sentence, num_cor = self.apply_predictions(sentence, preds)
        log.debug("number of changes: {}".format(num_cor))
    ...
    def apply_predictions(self, sentence, all_preds):
        ...
        for cword, preds in all_preds:
            pred = self.__predicted_word(cword, preds, self.threshold)
        ...
        if self.restore_case and new_sentence != sentence:
            new_sentence = restore_sentence_case(new_sent, sent)

        return (new_sentence, c)

    def __predicted_word(self, cword, preds, thr=None):
        # predictions are already sorted from best to worse
        pred_cw, conf = preds[0]
        if (pred_cw == cword.src) or (thr and conf < thr):
            return cword.err
        # TODO: restore case
